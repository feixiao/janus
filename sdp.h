...
/*! \brief 处理已经被解析为janus_sdp结构的SDP信息的方法
 * \details 这个方法将处理来自对端的SDP信息，并相应地设置ICE候选项 
 ...
 * @param[in] handle janus_ice_handle
 * @param[in] sdp 需要被处理的sdp信息
 * @param[in] update 是否为更新sdp
 * @returns 0 in case of success, -1 in case of an error */
int janus_sdp_process(void *handle, janus_sdp *sdp, gboolean update);

/*! \brief 这个方法将解析一个由对端提供的远程候选者，不管它是否在缓慢地进行。
 * \details This method will parse a single remote candidate provided by a peer, whether it is trickling or not
 * @param[in] stream 候选者相关的janus_ice_stream对象
 * @param[in] candidate 需要被处理的候选者
 * @param[in] trickle 是否来自trickle或者sdp
