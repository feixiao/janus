...
/*! \brief Janus SDP的内部表示对象 */
typedef struct janus_sdp {
	/*! \brief v=  	#版本协议 */
	int version;
	/*! \brief o= name #用户名 */
	char *o_name;
	/*! \brief o= session ID  #会话标识符 */
	guint64 o_sessid;
	/*! \brief o= version  #会话版本 */
	guint64 o_version;
	/*! \brief o= protocol #地址类型 */
	gboolean o_ipv4;
	/*! \brief o= address  #单播地址 */
	char *o_addr;
	/*! \brief s=  #会话的名字 */
	char *s_name;
	/*! \brief t= start #开始时间 */
	...
	/*! \brief t= stop #结束时间 */
	guint64 t_stop;
	/*! \brief c= protocol # 连接协议 (not rendered for WebRTC usage) */
	gboolean c_ipv4;
	/*! \brief c= address # 连接地址 (not rendered for WebRTC usage) */
	char *c_addr;
	/*! \brief List of global a= attributes # 属性信息 */
	GList *attributes;
	/*! \brief List of m= m-lines #媒体描述 */
	GList *m_lines;
} janus_sdp;
...
typedef struct janus_sdp_mline {
	...
	/*! \brief payload类型的列表 */
	GList *ptypes;
	/*! \brief janus_sdp结构中的c_ipv4，表示连接协议 */
	gboolean c_ipv4;
	/*! \brief janus_sdp结构中的c_addr，表示连接协议 */
	char *c_addr;
	/*! \brief 带宽类型 */
	char *b_name;
	/*! \brief 带宽字节 */
...
/*! \brief 针对提供的janus_sdp offer生成janus_sdp answer, 使用多个参数决定如何提供返回值
 * (e.g., in terms of media to accept, reject, directions, etc.). Variable
 * arguments are in the form of a sequence of name-value terminated by a JANUS_SDP_OA_DONE, e.g.:
 \verbatim
