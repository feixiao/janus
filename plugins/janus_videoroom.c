...
typedef struct janus_videoroom_participant {
	...
	janus_videoroom *room;	/* Room 关联房间*/
	guint64 room_id;	/* Unique room ID */
...
// 维护janus_videoroom_participant和janus_videoroom_listener的关系
	...
	while(g_atomic_int_get(&initialized) && !g_atomic_int_get(&stopping)) {
		...
		if(session->participant_type == janus_videoroom_p_type_none) {
			...
			if(!strcasecmp(ptype_text, "publisher")) {
				...
				// 创建数据发送对象janus_videoroom_participant,关联数据
				...
				publisher->audio_ssrc = janus_random_uint32(); // SDP中需要使用到
				...
				json_object_set_new(event, "private_id", json_integer(publisher->pvt_id));
				 // 返回已经存在的publisher给客户端，他需要创建响应的listener对象来获取数据
				...
				if(publisher == NULL || publisher->sdp == NULL) {
					...
					// 数据转发是基于 发布和订阅模型，接收到rtp包之后通过轮询监听者完成转发。
					janus_mutex_lock(&publisher->listeners_mutex);
					...
					if(publisher->sdp != NULL) {
						...
						// janus_plugin_push_event内部会修改sdp信息
		...
		if(!msg_sdp) {
			...
			if(session->participant_type != janus_videoroom_p_type_publisher) {
				...
				if(res != JANUS_OK) {
					...
					if(participant->sdp)
						g_free(participant->sdp);
					participant->sdp = offer_sdp; // 保存participant的sdp
