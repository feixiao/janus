...
// janus_plugin_push_event中调用janus_plugin_handle_sdp处理返回给客户端的SDP信息
...
// 创建Session， session_id为0就内部自己创建一个
...
// 根据session_id从hash_table里面获取Session对象指针
...
int janus_process_incoming_request(janus_request *request) {
	...
	// 请求第一次进入还未分配session_id 和 handle_id
	if(session_id == 0 && handle_id == 0) {
		...
		// 这边只处理info、ping、create命令，所以如果这边不是create就可以退出了
	...
	/* keepalive、attach、destroy、detach、hangup、message处理 */
	if(!strcasecmp(message_text, "keepalive")) {
		...
		int error = 0;
		// 插件、Transport Session和ICE实例建立关联关系
		...
		if(jsep != NULL) {
			...
			if(!janus_flags_is_set(&handle->webrtc_flags, JANUS_ICE_HANDLE_WEBRTC_READY)
				...
				/* New session */
				if(offer) {
					/* Setup ICE locally (we received an offer) 创建libnice agent对象，设置相关的回调函数*/
				...
				// 根据对端sdp信息，设置janus_ice_handle配置
...
void janus_transport_incoming_request(janus_transport *plugin, void *transport, void *request_id, gboolean admin, json_t *message, json_error_t *error) {
	...
	g_async_queue_push(requests, request); 	// glib实现
}
...
gint main(int argc, char *argv[])
{
	...
	/* Start the sessions watchdog 类似定时器定期检查会话*/
	...
	// 加载位于/opt/janus/lib/janus/plugins下面的插件
	...
	/* 加载传输方式REST、WebSocket，Unix Socket... */
	...
	if(item && item->value)
		path = (char *)item->value;
	// 传输方式插件位置 /opt/janus/lib/janus/transports
