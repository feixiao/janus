...
static struct lws_protocols ws_protocols[] = {
	{ "http-only", janus_websockets_callback_http, 0, 0 },		// 告知客户端不能使用HTTP协议，需要使用WebSocket协议
...
/* WebSockets  */
//  This endpoint cannot be used for HTTP,告知客户端不能使用HTTP协议，需要使用WebSocket协议
static int janus_websockets_callback_http(
		struct lws *wsi,
		enum lws_callback_reasons reason,
		void *user, void *in, size_t len)
		...
		void *user, void *in, size_t len, gboolean admin)
{
	...
	// 总结使用libwebsockets开发接入层  https://blog.csdn.net/qifengzou/article/details/50281545
	switch(reason) {
		// ws连接建立完成
