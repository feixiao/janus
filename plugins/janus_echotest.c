...
// janus_plugin_session中获取plugin_handle(获取指向会话的指针)
...
// 插件create_session的具体实现，attach命令中完成ice_handle,Session和Plugin关联关系建立之后调用
void janus_echotest_create_session(janus_plugin_session *handle, int *error) {
	...
	// 保持Session指针到janus_plugin_session
...
// 回调函数，处理transport接收到的Message消息
struct janus_plugin_result *janus_echotest_handle_message(janus_plugin_session *handle, char *transaction, json_t *message, json_t *jsep) {
	if(g_atomic_int_get(&stopping) || !g_atomic_int_get(&initialized))
		return janus_plugin_result_new(JANUS_PLUGIN_ERROR, g_atomic_int_get(&stopping) ? "Shutting down" : "Plugin not initialized", NULL);

	// 构建janus_echotest_message对象
